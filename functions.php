...
class userMeta{
	...
	/**
	 * 主要用作增加挂钩，初始化metabox以及保存文章同时保存metabox的动作
	*/
	 ...
	 * @param $post 文章对象
	*/
	public function createMetaBoxStyle($post){

	    foreach($this->metaData as $meta_box) {
	        $meta_box_value = get_post_meta($post->ID, 'tt_'.$meta_box['name'], true);
	        if($meta_box_value == "")
	        ...
	        case 'textarea' :
						echo'<h4>'.$meta_box['title'].'</h4><textarea cols="60" rows="3" name="tt_'.$meta_box['name'].'">'.$meta_box_value.'</textarea><br />';
					break;
					case 'text' :
						echo'<h4>'.$meta_box['title'].'</h4><input type="text" name="tt_'.$meta_box['name'].'"value="'.$meta_box_value.'" /><br />';
					break;
					case 'checkbox' : 
					if($meta_box_value=='1'){
						echo "<input type=\"checkbox\" id=\"tt_{$meta_box['name']}\" name=\"tt_{$meta_box['name']}\" value=\"1\" checked=\"checked\" /><label for=\"tt_{$meta_box['name']}\">{$meta_box['title']}</label>";
					}else{
						echo "<input type=\"checkbox\" id=\"tt_{$meta_box['name']}\" name=\"tt_{$meta_box['name']}\" value=\"1\" /><label for=\"tt_{$meta_box['name']}\">{$meta_box['title']}</label>";					
					}
					break;
					case 'title' :
						echo '<h4>'.$meta_box['name'].'</h4><br/>';
					break;
	        }
	    }
	}
	...
	public function createMetaBox() {
	    add_meta_box( $this->name, $this->zhname, array( &$this, 'createMetaBoxStyle'), 'post', 'normal', 'high');
	}
	...
	*/
	public function saveMeta(){
		...
		if( !($post_id = (int) $_POST['post_ID']) ) return False;
	    ...
	    foreach($this->metaData as $meta_box) {
	        ...
	        $data = $_POST['tt_'.$meta_box['name']];

	        if(get_post_meta($post_id, 'tt_'.$meta_box['name']) == "")
	            add_post_meta($post_id, 'tt_'.$meta_box['name'], $data, true);

	        elseif($data != get_post_meta($post_id, 'tt_'.$meta_box['name'], true))
	            update_post_meta($post_id, 'tt_'.$meta_box['name'], $data);

	        elseif($data == "")
	            delete_post_meta($post_id, 'tt_'.$meta_box['name'], get_post_meta($post_id, 'tt_'.$meta_box['name'], true));
	    }
